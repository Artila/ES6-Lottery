...
var makeIssue = function() {
	...
	var cd = cur_date.getTime();
	// 每天销售时间：9:00~22:00，第一期就是9:00~9:10，十分钟一期，一共78期
	...
	var fid = first_issue_date.getTime();
	// 截止时间
	var end_issue_date = new Date(fid + 77*10*60*1000);
	var eid = end_issue_date.getTime();
	
	var cur_issue, end_time, state;
	// 正常销售: 9:00~22:00
	if(cd - fid > 0 && cd - eid < 0) {
		// 第一期
		var cur_issue_date = new Date();
		cur_issue_date.setHours(9);
		cur_issue_date.setMinutes(0);
		cur_issue_date.setSeconds(0);		
		var cid = cur_issue_date.getTime();
		
		var minus_time = cd - cid;
		// 向上取整，现在是一天中的第几期，截止时间是end_time
		var h = Math.ceil(minus_time/1000/60/10);
		var end_date = new Date(cid + h*10*60*1000);		
		end_time = end_date.getTime();
		
		// 完整期号：年月日+一天中的第几期,例如2017010178期
		cur_issue = [end_date.getFullYear(), ('0' + (end_date.getMonth() + 1)).slice(-2), ('0' + end_date.getDate()).slice(-2), ('0' + h).slice(-2)].join('');
				
	} else {
		// 销售已截止，这里判断的是22:00之后的情况，买的是明天的第一期
		// 否则的话就是销售还未开始，买的是当天的第一期，first_issue_date 还是今天第一期，不用改。
		if (cd - eid > 0) {
	...
	// 判断状态：一期10分钟，最后2分钟是开奖时间
	if(end_time - cd > 2*60*1000) {
		state = '正在销售';
	} else {
		state = '开奖中';
	}
